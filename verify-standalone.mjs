...
assert.equal(get('#title').textContent,'Hoop League');assert.equal(get('#teams').children.filter(n=>n.tag==='button').length,32);assert.equal(get('#assetCount').textContent,111);
...
console.log('Standalone smoke check passed: exported HTML starts with no fetches, 32 team entries and 111 embedded images.');
